...
const StockCard: React.FC<StockCardProps> = ({ stock }) => {
  ...
  const fmtNum = (val: number | null) => val !== null ? val.toFixed(2) : 'N/A';
  
  const getPegColor = (peg: number | null) => {
    if (peg === null) return 'text-slate-500';
  ...
  const getRoeColor = (roe: number | null) => {
    if (roe === null) return 'text-slate-500';
    return roe > 0.15 ? 'text-blue-400 font-bold' : 'text-slate-300';
  };

  return (
    <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-5 shadow-xl hover:border-blue-500/40 transition-all group backdrop-blur-sm">
      <div className="flex justify-between items-start mb-4">
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-xl font-black text-white group-hover:text-blue-400 transition-colors">{stock.symbol}</h3>
          ...
          <p className="text-[11px] text-slate-500 truncate w-40" title={stock.shortName}>{stock.shortName}</p>
        </div>
        <div className="text-right">
          <p className="text-lg font-mono font-bold text-white">
            {stock.currentPrice.toFixed(2)}
            <span className="text-[10px] text-slate-500 ml-1 uppercase">{stock.currency}</span>
          </p>
          <p className="text-[10px] text-slate-600 font-mono">{stock.industry}</p>
        </div>
      </div>
